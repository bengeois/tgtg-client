...
describe('getFavorites', () => {
  test('should throw error because authentication is required', async () => {
    ...
    await expect(() => client.getFavorites()).rejects.toThrowError(
  ...
  test('should throw error 422', async () => {
    //@ts-ignore
    nock(TgtgConfig.baseUrl).post(`/${TgtgConfig.items}`, requestBodies.favorites).reply(422);
    ...
    await expect(() => client.getFavorites(10, 0)).rejects.toThrowError(
  ...
  test('should throw error because no items were returned', async () => {
    //@ts-ignore
    nock(TgtgConfig.baseUrl).post(`/${TgtgConfig.items}`, requestBodies.favorites).reply(200, {});
    nock(TgtgConfig.baseUrl)
      .post(`/${TgtgConfig.authPolling}`, requestBodies.authenticate)
      .reply(200, responseBodies.authenticate);
  ...
  test('should return favorites', async () => {
    //@ts-ignore
    nock(TgtgConfig.baseUrl).post(`/${TgtgConfig.items}`, requestBodies.favorites).reply(200, responseBodies.favorites);
    nock(TgtgConfig.baseUrl)
      .post(`/${TgtgConfig.authPolling}`, requestBodies.authenticate)
      .reply(200, responseBodies.authenticate);
    ...
    await expect(client.getFavorites(10, 0)).resolves.toEqual(get(responseBodies.favorites, 'items'));
  });
});
